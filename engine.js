/* ============================================================
   Brand Archetype OS — Engine Layer
   ============================================================
   Содержит:
   - Глобальное состояние (userVector, currentSystemLayer, etc.)
   - Функции расчёта distance и rankings
   - Управление системным overlay и слоями
   - Drag & Drop поля (field / preview)
   - Drag & Drop кривых синтеза
   - updateAll / updateConstruction / generateSite
   - Scroll Reveal и Nav Scroll
   - Инициализация
   ============================================================ */
...
function goToLayer(n) {
  document
    .querySelectorAll(".system-layer")
    .forEach((l, i) => l.classList.toggle("active", i === n));
  document
...
function initFieldZones() {
  ...
  const positions = [
    { t: 8, l: 8, c: "rgba(231,76,60,0.12)" },
    { t: 8, r: 8, c: "rgba(155,89,182,0.12)" },
    { b: 8, l: 8, c: "rgba(243,156,18,0.12)" },
    { b: 8, r: 8, c: "rgba(39,174,96,0.12)" },
    { t: 45, l: 45, c: "rgba(233,30,99,0.08)" },
  ];
  zones.innerHTML = positions
    .map((p, i) => {
...
function setupFieldDrag(containerId, dotId, isPreview) {
  ...
  dot.addEventListener("mousedown", (e) => {
    isDragging = true;
    dragTarget = isPreview ? "preview" : "field";
    e.preventDefault();
  });
...
function setupCurveDrag() {
  document.querySelectorAll(".synthesis-curve-track").forEach((track) => {
    ...
    function updateFromX(cx) {
      ...
      const pct = Math.max(
        0,
        Math.min(100, ((cx - rect.left) / rect.width) * 100),
      );
...
function updateAll() {
  ...
  if (fconf)
    fconf.style.width = Math.max(5, 100 - r.primary.distance / 3) + "%";
  ...
  if (pconf)
    pconf.style.width = Math.max(5, 100 - r.primary.distance / 3) + "%";
...
function highlightComp(el) {
  document
    .querySelectorAll(".construction-component")
